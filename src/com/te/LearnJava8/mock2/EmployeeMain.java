package com.te.LearnJava8.mock2;
...
//		List<Employee> sortedEmployees = employees.stream().filter(employee->(employee.getJobTitle().equals("Manager") && employee.getSalary()>=100000 && 
//				( employee.getDateOfJoining().getYear()-LocalDate.now().getYear())<=3))
//		 .sorted((e1,e2)->Integer.compare(e2.getYearsOfExperience(), e1.getYearsOfExperience()))
//		 .sorted((e1,e2)->Integer.compare(e1.getPerformanceRating(), e2.getPerformanceRating()))
//		 .collect(Collectors.toList());
//		sortedEmployees.stream().forEach(employee->System.out.println(employee.getName().toUpperCase()));
		
		
		
		List<Employee> sortedEmployees2 = employees.stream()
		.filter(employee->(employee.getYearsOfExperience()<3 && employee.getDepartment()!="IT" && employee.getPerformanceRating()>=8))
		.sorted((e1,e2)->Integer.compare(e2.getPerformanceRating(), e1.getPerformanceRating()))
		.sorted((e1,e2)->Integer.compare(e1.getYearsOfExperience(), e2.getYearsOfExperience()))
		.collect(Collectors.toList());
		System.out.println(sortedEmployees2);
		
		sortedEmployees2.forEach(employee->System.out.println(employee.getName()+employee.getAge()));
