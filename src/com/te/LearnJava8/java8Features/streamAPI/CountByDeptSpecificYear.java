...
public class CountByDeptSpecificYear {
	public static void main(String[] args) {
		...
		Map<String, List<Employee>> filteredEmployees = employees.stream().filter(employee->employee.getEmployee_joiningDate().getYear()==2020)
		.collect(Collectors.groupingBy(Employee::getDept_name));
		
		
		filteredEmployees.forEach((dept,eList)->{
			System.out.println("Employees present in "+dept+" are "+eList.stream().count());
		});
	}
